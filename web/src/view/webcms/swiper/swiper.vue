  ...
  <div>
    ...
    <div class="gva-table-box">
        <div class="gva-btn-list">
            ...
            <template #reference>
                <el-button icon="delete" style="margin-center: 10px;" :disabled="!multipleSelection.length" @click="deleteVisible = true">删除</el-button>
            </template>
            </el-popover>
        </div>
        <el-table
        ...
        <el-table-column align="center" label="描述" prop="desc" width="300" />
        <el-table-column align="center" label="轮播图" prop="thumb" width="180" >
          <template #default="scope">
            <CustomPic style="margin-top:8px" :pic-src="scope.row.thumb" pic-type="swiper" />
          </template>
        </el-table-column>
        <el-table-column align="center" label="手机端轮播图" prop="wapThumb" width="180" >
          <template #default="scope">
            <CustomPic style="margin-top:8px" :pic-src="scope.row.wapThumb" pic-type="swiper" />
          </template>
        </el-table-column>
        <el-table-column align="center" label="跳转链接" prop="url" width="200" />
        ...
        <el-table-column align="center" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateSwiperFunc(scope.row)">修改</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="轮播图:"  prop="thumb" >
          ...
          <div style="display:inline-block" @click="openHeaderChange('thumb')">
              <img v-if="formData.thumb" alt="轮播图" class="header-img-box" :src="(formData.thumb && formData.thumb.slice(0, 4) !== 'http')?path+formData.thumb:formData.thumb">
              <div v-else class="header-img-box">从媒体库选择</div>
            </div>
        </el-form-item>
        <el-form-item label="wap轮播图:"  prop="wapThumb" >
          <!-- <el-input v-model="formData.thumb" :clearable="true"  placeholder="请输入" /> -->
          <div style="display:inline-block" @click="openHeaderChange('wapThumb')">
              <img v-if="formData.wapThumb" alt="wap轮播图" class="header-img-box" :src="(formData.wapThumb && formData.wapThumb.slice(0, 4) !== 'http')?path+formData.wapThumb:formData.wapThumb">
              <div v-else class="header-img-box">从媒体库选择</div>
            </div>
        </el-form-item>
