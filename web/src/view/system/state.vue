    ...
    <template v-else>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
        <div
          ...
          :key="tile.key"
          class="gva-card bg-container px-4 py-3"
        >
          ...
          <div class="mt-0.5 text-2xl font-semibold leading-snug" :class="tile.valueClass">
            {{ tile.value }}<span v-if="tile.suffix" class="text-xs font-normal text-muted-foreground ml-1">{{ tile.suffix }}</span>
          </div>
          <!-- 趋势微线: 本地累积快照, 不足 2 个时留白占位(高度不塌) -->
          <svg
            v-if="tile.points"
            class="block mt-1.5 w-full"
            :class="tile.lineClass"
            height="22"
            viewBox="0 0 120 22"
          ...
          <template v-if="disks.length">
            <div v-for="d in disks" :key="d.mountPoint" class="mb-3 last:mb-0">
              <div class="flex justify-between text-xs mb-1">
                ...
                <span class="text-muted-foreground tabular-nums">{{ d.usedGb }} / {{ d.totalGb }} GB · {{ d.usedPercent }}%</span>
              </div>
