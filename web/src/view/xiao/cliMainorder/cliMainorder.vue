  ...
  <div>
    ...
    <div class="gva-table-box">
        <div class="gva-btn-list">
            ...
            <ExportTemplate  template-id="xiao_CliMainorder" />
            <ExportExcel  template-id="xiao_CliMainorder" />
            <ImportExcel  template-id="xiao_CliMainorder" @on-success="getTableData" />
        </div>
        <el-table
        ref="multipleTable"
        style="width: 100%"
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateCliMainorderFunc(scope.row)">变更</el-button>
    ...
    <el-drawer destroy-on-close size="800" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="团队订单:"  prop="num" >
              <el-input v-model.number="formData.num" :clearable="true" placeholder="请输入当前订单" />
            </el-form-item>
            <el-form-item label="团队总额:"  prop="amount" >
    ...
    <el-drawer destroy-on-close size="800" v-model="detailShow" :show-close="true" :before-close="closeDetailShow">
            <el-descriptions :column="1" border>
                    ...
                    <el-descriptions-item label="团队订单">
                        {{ detailFrom.num }}
                    </el-descriptions-item>
                    <el-descriptions-item label="团队总额">
                        {{ detailFrom.amount }}
                    </el-descriptions-item>
...
import {
  createCliMainorder,
  deleteCliMainorder,
  deleteCliMainorderByIds,
  updateCliMainorder,
  findCliMainorder,
  getCliMainorderList
} from '@/api/xiao/cliMainorder'
...
const getTableData = async() => {
  const table = await getCliMainorderList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteCliMainorderByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCliMainorderFunc = async(row) => {
    const res = await findCliMainorder({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteCliMainorderFunc = async (row) => {
    const res = await deleteCliMainorder({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createCliMainorder(formData.value)
                  break
                case 'update':
                  res = await updateCliMainorder(formData.value)
                  break
                default:
                  res = await createCliMainorder(formData.value)
                  break
