<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束时间"></el-date-picker>
        ...
        <el-form-item label="款式id">
             ...
             <el-input v-model.number="searchInfo.styleID" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="款式id" prop="styleID" width="120" />
        <el-table-column align="left" label="颜色" prop="color" width="120" />
        <el-table-column align="left" label="长度" prop="length" width="120" />
        <el-table-column align="left" label="尺码" prop="size" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateCroppingRecordFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteCroppingRecord,
  deleteCroppingRecordByIds,
  updateCroppingRecord,
  findCroppingRecord,
  getCroppingRecordList
...
const getTableData = async() => {
  const table = await getCroppingRecordList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteCroppingRecordByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCroppingRecordFunc = async(row) => {
    const res = await findCroppingRecord({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteCroppingRecordFunc = async (row) => {
    const res = await deleteCroppingRecord({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
