  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateRechargeOptionFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getRechargeOptionList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteRechargeOptionByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateRechargeOptionFunc = async(row) => {
    const res = await findRechargeOption({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteRechargeOptionFunc = async (row) => {
    const res = await deleteRechargeOption({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
