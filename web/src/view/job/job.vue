<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束时间"></el-date-picker>
           ...
           <el-form-item label="步骤" prop="step">
            <el-select v-model="searchInfo.step" clearable placeholder="请选择" @clear="()=>{searchInfo.step=undefined}">
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="裁剪单id" prop="croppingID" width="120" />
        <el-table-column align="left" label="用户id" prop="userID" width="120" />
        <el-table-column align="left" label="工序名称" prop="processName" width="120" />
        <el-table-column align="left" label="单价" prop="price" width="120" />
        <el-table-column align="left" label="申请数量" prop="quantity" width="120" />
        <el-table-column align="left" label="预估收入" prop="income" width="120" />
        <el-table-column align="left" label="真实数量" prop="realQuantity" width="120" />
        <el-table-column align="left" label="真实收入" prop="realIncome" width="120" />
        <el-table-column align="left" label="步骤" prop="step" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.step,stepOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="工作类型" prop="jobType" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.jobType,jobTypeOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateJobFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="预估收入:"  prop="income" >
          <el-input-number v-model="formData.income"  style="width:100%" :precision="2" :clearable="true"  />
        </el-form-item>
        ...
        <el-form-item label="真实收入:"  prop="realIncome" >
          <el-input-number v-model="formData.realIncome"  style="width:100%" :precision="2" :clearable="true"  />
        </el-form-item>
        <el-form-item label="步骤:"  prop="step" >
          <el-select v-model="formData.step" placeholder="请选择" style="width:100%" :clearable="true" >
            <el-option v-for="(item,key) in stepOptions" :key="key" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="工作类型:"  prop="jobType" >
          <el-select v-model="formData.jobType" placeholder="请选择" style="width:100%" :clearable="true" >
