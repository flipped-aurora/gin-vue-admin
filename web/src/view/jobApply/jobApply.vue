  ...
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束时间"></el-date-picker>
        ...
        <el-form-item label="裁剪单id">
            
             <el-input v-model.number="searchInfo.croppingID" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="工序名" prop="processName" width="120" />
        <el-table-column align="left" label="价格" prop="price" width="120" />
        <el-table-column align="left" label="数量" prop="quantity" width="120" />
        <el-table-column align="left" label="计价类型" prop="jobType" width="120">
        ...
        <el-table-column align="left" label="审核状态" prop="status" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.status,handleStatusOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateJobApplyFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getJobApplyList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteJobApplyByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateJobApplyFunc = async(row) => {
    const res = await findJobApply({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteJobApplyFunc = async (row) => {
    const res = await deleteJobApply({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
