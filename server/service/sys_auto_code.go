...
func addAutoMoveFile(data *tplData) {
	...
	if strings.Contains(fileSlice[1], "server") {
		...
		if strings.Contains(fileSlice[n-1], "js") {
			...
			data.autoMoveFilePath = filepath.Join("../", "web", "src", "view",
				filepath.Base(filepath.Dir(filepath.Dir(data.autoCodePath))), strings.TrimSuffix(base, filepath.Ext(base))+"WorkFlowForm.vue")
		} else if strings.Contains(fileSlice[n-2], "form") {
			data.autoMoveFilePath = filepath.Join("../", "web", "src", "view",
				filepath.Base(filepath.Dir(filepath.Dir(data.autoCodePath))), strings.TrimSuffix(base, filepath.Ext(base))+"Form.vue")
