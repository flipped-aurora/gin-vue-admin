...
func (a *Base) Parse(filename string, writer io.Writer) (file *ast.File, err error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, global.Translate("utils.fileOpenParseFailed"), filename)
	}
	return file, nil
}
...
func (a *Base) Format(filename string, writer io.Writer, file *ast.File) error {
	...
	if writer == nil {
		...
		defer open.Close()
		if err != nil {
			return errors.Wrapf(err, global.Translate("utils.fileOpenFailed"), filename)
		}
		writer = open
	}
	err := format.Node(writer, fileSet, file)
	if err != nil {
		return errors.Wrapf(err, global.Translate("utils.injectionFailed"), filename)
	}
	return nil
}
