<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="是否换乘.0:false;1:true" prop="isTransfer" width="120">
            <template #default="scope">{{ formatBoolean(scope.row.isTransfer) }}</template>
        </el-table-column>
        <el-table-column align="left" label="是否占座.0:false;1:true" prop="isOccupySeat" width="120" />
        <el-table-column align="left" label="完成状态.0:未完成;1:出票成功;2:出票失败" prop="completeStatus" width="120" />
        <el-table-column align="left" label="出票失败原因" prop="failReason" width="120" />
        <el-table-column align="left" label="设备名称" prop="machineName" width="120" />
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" :title="type==='create'?'添加':'修改'" destroy-on-close>
      <el-scrollbar height="500px">
          <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="是否换乘.0:false;1:true:"  prop="isTransfer" >
            ...
            <el-form-item label="是否占座.0:false;1:true:"  prop="isOccupySeat" >
              <el-input v-model.number="formData.isOccupySeat" :clearable="true" placeholder="请输入" />
            </el-form-item>
            <el-form-item label="完成状态.0:未完成;1:出票成功;2:出票失败:"  prop="completeStatus" >
    ...
    <el-dialog v-model="detailShow" style="width: 800px" lock-scroll :before-close="closeDetailShow" title="查看详情" destroy-on-close>
      <el-scrollbar height="550px">
        <el-descriptions column="1" border>
                ...
                <el-descriptions-item label="订单取票号">
                    {{ formData.ticketNumber }}
                </el-descriptions-item>
                <el-descriptions-item label="订单联系电话">
                    {{ formData.contactPhone }}
                </el-descriptions-item>
                <el-descriptions-item label="是否换乘.0:false;1:true">
                    {{ formData.isTransfer }}
                </el-descriptions-item>
                <el-descriptions-item label="是否占座.0:false;1:true">
                    {{ formData.isOccupySeat }}
                </el-descriptions-item>
                <el-descriptions-item label="完成状态.0:未完成;1:出票成功;2:出票失败">
                    {{ formData.completeStatus }}
                </el-descriptions-item>
                <el-descriptions-item label="出票失败原因">
                    {{ formData.failReason }}
                </el-descriptions-item>
...
const getTableData = async() => {
  const table = await getOrderInfoList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteOrderInfoByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateOrderInfoFunc = async(row) => {
    const res = await findOrderInfo({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteOrderInfoFunc = async (row) => {
    const res = await deleteOrderInfo({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
