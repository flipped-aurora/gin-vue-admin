    ...
    <div class="gva-search-box">
      ...
      >
        ...
        <el-form-item label="角色ID" prop="roleid">
          <el-input v-model="searchInfo.roleidsearch" placeholder="搜索条件" />
        </el-form-item>
        <el-form-item label="机巢列表" prop="nestid">
          <el-input v-model="searchInfo.nestid" placeholder="搜索条件" />
        </el-form-item>
        <el-form-item>
        ...
        <el-table-column align="left" label="操作">
          <template #default="scope">
            <el-button
              ...
              icon="edit"
              class="table-button"
              @click="updateNestRoleFunc(scope.row)"
              >变更</el-button
            >
            <el-button
              type="primary"
...
const getTableData = async () => {
  const table = await getNestRoleList({
    page: page.value,
...
const onDelete = async () => {
  ...
  const res = await deleteNestRoleByIds({ ids });
  if (res.code === 0) {
    ElMessage({
      type: "success",
      message: "删除成功",
...
// 更新行
const updateNestRoleFunc = async (row) => {
...
// 删除行
const deleteNestRoleFunc = async (row) => {
  const res = await deleteNestRole({ ID: row.ID });
  if (res.code === 0) {
    ElMessage({
      type: "success",
      message: "删除成功",
