<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="center" label="名称" prop="title" width="120" />
        <el-table-column align="center" label="跳转链接" prop="url" width="240" />
        <el-table-column align="center" label="网站logo" prop="thumb" width="120" >
          <template #default="scope">
            <CustomPic style="margin-top:8px" :pic-src="scope.row.thumb" pic-type="swiper" />
          </template>
        </el-table-column>
        <el-table-column align="center" label="显示类型" prop="type" width="120" >
          <template #default="scope">{{ typeBoolean(scope.row.type) }}</template>
        </el-table-column>
        ...
        <el-table-column align="center" label="操作">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateLinksFunc(scope.row)">修改</el-button>
      ...
      <el-form :model="formData" label-position="right" :inline="true" ref="elFormRef" :rules="rule" label-width="90px">
        ...
        <el-form-item label="显示类型:"  prop="type" >
            <el-select v-model="formData.type" placeholder="请选择" style="width:100%" :clearable="true" >
               <el-option v-for="item in linkstype" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
        </el-form-item>
...
const typeBoolean = (val) => {
  for ( var i=0;i<linkstype.value.length;i++) {
    if (linkstype.value[i].value == val) { 
        return linkstype.value[i].label
    }
  }
}
// 自动化生成的字典（可能为空）以及字段
