...
func (menuService *MenuService) getMenuTreeMap(authorityId []uint) (treeMap map[string][]system.SysMenu, err error) {
	...
	err = global.GVA_DB.Where("sys_authority_authority_id in (?)", authorityId).Find(&SysAuthorityMenus).Error
	if err != nil {
		return
	}
	...
	err = global.GVA_DB.Where("authority_id in (?)", authorityId).Preload("SysBaseMenuBtn").Find(&btns).Error
	if err != nil {
		return
	}
	...
	for _, v := range btns {
		...
		btnMap[v.SysMenuID][v.SysBaseMenuBtn.Name] = authorityId[0]
	}
