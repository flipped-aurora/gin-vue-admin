        ...
        <el-table-column align="left" label="操作" width="200" fixed="right">
          <template #default="scope">
            <el-button
              ...
              icon="edit"
              class="table-button"
              @click="updateNestInfoFunc(scope.row)"
              >变更</el-button
            >
            <el-button
              type="primary"
      ...
      >
        ...
        <el-row>
          <el-col :span="24" class="grid-cell">
            <el-form-item label="飞机推流地址" prop="aircraftVideoPushURL">
              <el-input
                v-model="formData.aircraftVideoPushURL"
                type="text"
                clearable
              ></el-input>
            </el-form-item>
        ...
        <el-row>
          <el-col :span="24" class="grid-cell">
            <el-form-item label="飞机直播地址" prop="aircraftVideoURL">
              <el-input
                v-model="formData.aircraftVideoURL"
                type="text"
                clearable
              ></el-input>
            </el-form-item>
...
import nestInfoForm from "./nestInfoForm.vue";
...
const getTableData = async () => {
  const table = await getAllNestInfoList({
    page: page.value,
...
const onDelete = async () => {
  ...
  const res = await deleteNestInfoByIds({ ids });
  if (res.code === 0) {
    ElMessage({
      type: "success",
      message: "删除成功",
...
// 更新行
const updateNestInfoFunc = async (row) => {
  const res = await findNestInfo({ ID: row.ID });
  type.value = "update";
  if (res.code === 0) {
...
// 删除行
const deleteNestInfoFunc = async (row) => {
  const res = await deleteNestInfo({ ID: row.ID });
  if (res.code === 0) {
    ElMessage({
      type: "success",
      message: "删除成功",
