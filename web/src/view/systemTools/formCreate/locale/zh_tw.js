  ...
  com: {
    cascader: {
      ...
      propsOpt: {
        ...
        emitPath:
          '在選中節點改變時，是否返回由該節點所在的各級菜單的值所組成的數組',
    ...
    elCollapse: {
      ...
      event: {
        change:
          '切換當前活躍面板，在手風琴模式下其類型是string，在其他模式下是array'
      },
      props: {
        accordion: '是否手風琴模式'
      }
    ...
    elTransfer: {
      ...
      props: {
        ...
        targetOrderInfo:
          '若為 original，則保持與數據相同的順序；若為 push，則新加入的元素排在最後；若為 unshift，則新加入的元素排在最前',
        titles: '自定義列表標題',
        buttonTexts: '自定義按鈕文案',
        props: '數據源的字段別名'
      }
    ...
    tree: {
      ...
      props: {
        ...
        defaultExpandAll: '是否默認展開所有節點',
        expandOnClickNode:
          '是否在點擊節點的時候展開或者收縮節點，如果為 false，則只有點箭頭圖標的時候才會展開或者收縮節點。',
        checkOnClickNode: '是否在點擊節點的時候選中節點',
        autoExpandParent: '展開子節點的時候是否自動展開父節點',
        checkStrictly:
          '在顯示複選框的情況下，是否嚴格的遵循父子不互相關聯的做法',
