...
func InitWorkflowProcessRouter(Router *gin.RouterGroup) {
	WorkflowProcessRouter := Router.Group("workflowProcess").Use(middleware.OperationRecord())
	{
		...
		WorkflowProcessRouter.GET("findWorkflowStep", v1.FindWorkflowStep)                        // 根据ID获取审批流步骤
		WorkflowProcessRouter.GET("getWorkflowProcessList", v1.GetWorkflowProcessList)            // 获取WorkflowProcess列表
		WorkflowProcessRouter.POST("startWorkflow", v1.StartWorkflow)                             // 开启审批流
		WorkflowProcessRouter.POST("completeWorkflowMove", v1.CompleteWorkflowMove)               // 提交审批流
		WorkflowProcessRouter.GET("getMyStated", v1.GetMyStated)                                  // 获取我发起的审批流
