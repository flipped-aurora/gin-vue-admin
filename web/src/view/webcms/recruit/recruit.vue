<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="招聘职位" prop="job" width="120" />
        <el-table-column align="left" label="招聘人数" prop="count" width="120" />
        <el-table-column align="left" label="工作地点" prop="address" width="120" />
        <el-table-column align="left" label="是否显示" prop="enable" width="120">
        ...
        <el-table-column align="left" label="操作">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateRecruitFunc(scope.row)">修改</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" :title="type==='create'?'添加内容':'修改内容'" destroy-on-close>
      <el-form :model="formData" label-position="right" ref="elFormRef" :inline="true" :rules="rule" label-width="90px">
        ...
        <el-form-item label="招聘职位:"  prop="job" style="width: 45%;">
          <el-input v-model="formData.job" :clearable="true"  placeholder="请输入" />
        </el-form-item>
        <el-form-item label="招聘人数:"  prop="count" style="width: 45%;">
          <el-input v-model="formData.count" :clearable="true"  placeholder="请输入" />
        </el-form-item>
        <el-form-item label="工作地点:"  prop="address" style="width: 45%;">
...
const getTableData = async() => {
  const table = await getRecruitList({ page: page.value, pageSize: pageSize.value,cateId:props.defaultCatid, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteRecruitByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateRecruitFunc = async(row) => {
    const res = await findRecruit({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteRecruitFunc = async (row) => {
    const res = await deleteRecruit({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
