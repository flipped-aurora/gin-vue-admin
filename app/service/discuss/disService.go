...
func (disDataService *DisInfoService) CreateDisInfo(disData *discuss.DisInfo) (err error) {
	err = mysqlDB.DB.Create(disData).Error
	return err
}
...
func (disDataService *DisInfoService) DeleteDisInfo(id string) (err error) {
	err = mysqlDB.DB.Delete(&discuss.DisInfo{}, "id = ?", id).Error
	return err
}
...
func (disDataService *DisInfoService) DeleteDisInfoByIds(ids []string) (err error) {
	err = mysqlDB.DB.Delete(&[]discuss.DisInfo{}, "id in ?", ids).Error
	return err
}
...
func (disDataService *DisInfoService) UpdateDisInfo(disData discuss.DisInfo) (err error) {
	err = mysqlDB.DB.Updates(&disData).Error
	return err
}
...
func (disDataService *DisInfoService) GetDisInfo(id string) (disData discuss.DisInfo, err error) {
	err = mysqlDB.DB.Preload("User").Preload("Com").Where("id = ?", id).First(&disData).Error
	return
}

// GetDisInfoInfoList 分页获取帖子信息记录
// Author [piexlmax](https://github.com/piexlmax)
func (disDataService *DisInfoService) GetDisInfoInfoList(info discuss.DisInfoSearch) (list []discuss.DisInfo, total int64, err error) {
	limit := info.PageSize
	offset := info.PageSize * (info.Page - 1)
	// 创建db
	db := mysqlDB.DB.Model(&discuss.DisInfo{})
...
func (this DisInfoService) GetInfo(id string) (reData interface{}, err error) {
	return this.GetDisInfo(id)
}
