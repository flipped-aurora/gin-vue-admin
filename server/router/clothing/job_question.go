...
func (s *JobQuestionRouter) InitJobQuestionRouter(Router *gin.RouterGroup) {
	...
	h5JobQuestionRouterWithoutRecord := Router.Group(global.GetAppApi() + "jobQuestion")
	...
	{
		h5JobQuestionRouterWithoutRecord.POST("createJobQuestion", jobQuestionApi.CreateJobQuestion)  // 新建JobQuestion
		h5JobQuestionRouterWithoutRecord.GET("findJobQuestion", jobQuestionApi.FindJobQuestion)       // 根据ID获取JobQuestion
		h5JobQuestionRouterWithoutRecord.GET("getJobQuestionList", jobQuestionApi.GetJobQuestionList) // 获取JobQuestion列表
		h5JobQuestionRouterWithoutRecord.PUT("handleJobQuestion", jobQuestionApi.HandleJobQuestion)   // 获取JobQuestion列表
	}
}
